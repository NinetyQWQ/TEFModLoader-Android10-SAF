...
import eternal.future.tefmodloader.utility.KEY_MOD_DIR_URI
import eternal.future.tefmodloader.utility.treeUriToPath
...
actual object HomeScreen{
    ...
    actual fun HomeScreen() {
        ...
        Scaffold(
                ...
                coroutineScope.launch {
                    try {
                        ...
                        withContext(Dispatchers.IO) {
                            ...
                            val gameAbis = Apk.getSupportedAbi(packageName)
                        ...
                        val errorMsg = locale.getString("initialization_failed") + ": ${e.localizedMessage}"
            ...
            // SAF 目录授权：用户通过系统目录选择器授权 mod 目录
            val modDirLauncher = rememberLauncherForActivityResult(
                ...
                treeUri?.let { uri ->
                    ...
                    configuration.setString(KEY_MOD_DIR_URI, uri.toString())
                    configuration.setString(KEY_MOD_DIR_PATH, treeUriToPath(uri))
                    EFLog.i("SAF 已授权 mod 目录: ${treeUriToPath(uri)}")
                }
                pendingExternalPackage?.let { pkg ->
                    pendingExternalPackage = null
                    launchExternalGame(pkg)
                }
            }
                                            ...
                                            Row(
                                                modifier = Modifier
                                                    ...
                                                    .clickable(enabled = !isLoading) {
                                                        ...
                                                        when (mode) {
                                                            0 -> {
                                                                // 外部模式：需先通过 SAF 授权 mod 目录
                                                                if (configuration.getString(KEY_MOD_DIR_PATH).isEmpty()) {
                                                                    pendingExternalPackage = packageName
                                                                    modDirLauncher.launch(null)
                                                                } else {
                                                                    launchExternalGame(packageName)
                                                                }
                                                            }
                                                        }
                                                    }
